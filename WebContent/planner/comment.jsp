...
$(document).ready(function(){
	...
	$.ajax({
		...
		dataType: 'json',
		url: "<%= root %>/comment",
    	data: { act:"commentList", p_id: "<%=p_id%>" },
...
function commentWrite() {
	...
	if($("#commentval").val() != "") {
		$.ajax({
			...
			dataType: 'json',
			url: "<%= root %>/comment",
	     	data: { act:"write", p_id: "<%=p_id%>", comment: $("#commentval").val() },
		    success:function(data) {
		    	$("#commentval").val("");
		    	commentList(data);
			},
...
function commentDelete(gc_id) {
	if(confirm("정말로 삭제하시겠습니까?")) {
		$.ajax({
			...
			dataType: 'json',
			url: "<%= root %>/comment",
	     	data: { act:"delete", p_id: "<%=p_id%>", gc_id: gc_id },
		    success:function(data) {
		    	alert("댓글이 삭제 되었습니다.");
		    	commentList(data);
			},
...
function commentModify(gc_id) {
	$.ajax({
		...
		dataType: 'json',
		url: "<%= root %>/comment",
     	data: { act:"modify", p_id: "<%=p_id%>", gc_id: gc_id, comment:  $("#commentModify" + gc_id).val() },
	    success:function(data) {
	    	alert("댓글이 수정 되었습니다.");
	    	commentList(data);
		},
...
<div class="container bg3">
		...
		<div class="row tabbed round">
			<ul>
				<a href="javascript:comment();"><li class="active">댓글</li></a>
				<%
				for(GroupMemberDto groupMemberDto : glist) {
					if(memberDto.getM_id().equals(groupMemberDto.getM_id())) {
				%>
